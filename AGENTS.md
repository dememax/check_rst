...
Navigate this repository's own `docs/` with the worktree implementation: use
`check_rst outline` when the target structure is unknown and `check_rst
context` when an exact heading or entry is known, then read the reported
physical range. A remembered heading, line number, or earlier full-file read
is not an exception. Never scan raw markup with `grep`/`head`/`tail`/`sed` to
rediscover structure. This project's own documentation makes the case against
that fallback; apply the discipline here first, not only to what the tool
checks for others.
